...
import com.example.pedulipantau.bluetooth.BluetoothLeService
...
import com.example.pedulipantau.model.LaporanHarian
import com.example.pedulipantau.model.MyCallback
    ...
    private val serviceConnection: ServiceConnection = object : ServiceConnection {
        ...
        override fun onServiceConnected(
            ...
            bluetoothService?.let { bluetooth ->
                ...
                Log.d(TAG, "onServiceConnected: Service Connected & Connect to Device")
...
//
    ...
    private fun changeLayoutData(data: List<String>?){
        ...
        tvOxygen.text = "$spo2 ${if(ir?.toInt()!! < 50000) "" else "%"}"
        tvTemperature.text = temp

    }

    private fun checkStatusLaporanHarian(){
        val lpModel = LaporanHarian(this)

        lpModel.getLatestLaporan(
            object: MyCallback {
                override fun onCallback(result: QueryDocumentSnapshot) {
                    var laporanDate = dateFormatting(result.data["created_at"].toString())
                    var currentDate = dateFormatting(LocalDateTime.now().toString())

                    val icon = findViewById<ImageView>(R.id.imageView6)
                    val judul = findViewById<TextView>(R.id.textView4)
                    val tanggal = findViewById<TextView>(R.id.textView5)
                    val btnAddLaporan = findViewById<Button>(R.id.button_add_laporan)

                    if(laporanDate == currentDate) {
                        icon.setImageResource(R.drawable.ic_checklist)
                        judul.text = "Laporan Sudah di Isi"
                        tanggal.text = "Update Terakhir Hari ini"
                        btnAddLaporan.visibility = View.GONE
                    } else {
                        icon.setImageResource(R.drawable.ic_caution)
                        judul.text = "Laporan Belum di Isi"
                        tanggal.text = "Update Terakhir ${laporanDate}"
                        btnAddLaporan.visibility = View.VISIBLE
                    }

//                    Log.d(TAG, "onCallback: ${laporanDate} -- ${currentDate}")
                }
            }
        )
    ...
    private fun dateFormatting(date: String): String{
        var parseDate = LocalDateTime.parse(date)
        var formatter = DateTimeFormatter.ofPattern("dd MMMM yyyy")
        var formattedDate = formatter.format(parseDate)
